...
namespace Hyt.DataAccess.Base
{
	internal class ActionsHandler
	{
		...
		internal void AutoMapColumnsAction<T>(params Expression<Func<T, object>>[] ignorePropertyExpressions)
		{
            ...
            //update:由从传入参数对象获取属性改为从泛型获取属性,避免子类属性干Sql字段
	...
	public interface ISelectBuilder<TEntity>
	{
        ...
        ISelectBuilder<TEntity> Distinct(string sql);
	...
	internal class SelectBuilder<TEntity> : ISelectBuilder<TEntity>
	{
        ...
        public ISelectBuilder<TEntity> Distinct(string sql)
        {
            Data.Distinct += sql;
            return this;
        }
	...
	internal class ExecuteQueryHandler
	{
		...
		private void PrepareDbCommand(bool useReader)
		{
			...
			{
                if(_command.Data.Context.Data.Provider is SqlServerProvider)
	...
	public partial class DbContext
	{
        ...
        {
            IDbProvider dbProvider = GetProviderFromConfig(connectionstringName);
        ...
        /// <returns></returns>
        /// <remarks>2014-09-01 杨浩 添加</remarks>
        private IDbProvider GetProviderFromConfig(string connectionStringName)
        {
            ...
            if (string.IsNullOrEmpty(settings.ProviderName))
                throw new FluentDataException("providerName is null");
            switch (settings.ProviderName)
            {
                ...
                case "Sqlite":
                    ...
                    break;
                case "SqlAzure":
                    dbProvider = new SqlAzureProvider();
                    break;
            ...
            if (dbProvider == null)
                throw new FluentDataException("dbprovider is null");
            return dbProvider;
        }
	}
	...
	internal static class ReflectionHelper
	{
		...
		private static Dictionary<string, PropertyInfo> BuildPropertyDictionary(Type type)
		{
			...
			foreach (var property in properties)
			{
                if (!result.ContainsKey(property.Name.ToLower())) //2016-3-24 杨浩 新增检查键值存在的限制防止反射的属性有出现重复
	...
	public class OracleProvider : IDbProvider
	{
	    ...
	    public IDbConnection CreateConnection()
	    {
	        var cnn = _dbProviderFactory.Value.CreateConnection();
	        //MiniProfiler性能分析
            bool isStart=false;
	        bool.TryParse(ConfigurationManager.AppSettings["MiniProfiler"],out isStart);
            if (isStart)
