...
namespace Extra.UpGrade.SDK.JingDong.Util
{
    /// <summary>
    /// 异步API下载工具类。
    /// </summary>
    public abstract class AtsUtils
    {
        ...
        public static string Download(string url, string destDir)
        {
            ...
            try
            {
                ...
                {
                    throw new JdException(wu.GetResponseAsString(rsp, Encoding.UTF8));
                }
            }
            catch (WebException e)
            {
                throw new JdException("isv.file-already-download", e.Message);
            }
            return file;
        }
        ...
        private static string GetFileName(string contentDisposition)
        {
            ...
            {
                throw new JdException("Invalid response header format!");
            }
        }
    }
}
