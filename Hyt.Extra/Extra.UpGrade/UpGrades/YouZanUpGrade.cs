...
using Extra.UpGrade.SDK.YouZan;
using Hyt.Model;
...
namespace Extra.UpGrade.UpGrades
{
    ...
    public class YouZanUpGrade : IUpGrade
    {
        private static readonly YouZanConfig config = UpGradeConfig.GetYouZanConfig();

        private static readonly KDTApiKit kit = new KDTApiKit(config.AppKey,config.AppSecret);

        /// <summary>
        /// 有赞支付类型转为系统支付类型
        /// </summary>
        /// <param name="payType">有赞支付类型</param>
        /// <returns></returns>
        /// <remarks>2016-7-15 杨浩 创建</remarks>
        public int YouZanPayToPayTypeSysNo(string payType)
        {
