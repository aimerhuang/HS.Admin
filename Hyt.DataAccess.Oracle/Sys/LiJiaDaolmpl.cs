...
namespace Hyt.DataAccess.Oracle.Sys
{
    ...
    /// 2013-11-10 罗熙 创建
    /// </remarks>
    public class LiJiaDaolmpl :ILiJiaDao
    {
        /// <summary>
        /// 添加LiJia同步日志
        /// </summary>
        ...
        public override int LiJiaSyncLogCreate(LiJiaSyncLog model)
        {
            return Context.Insert("LiJiaSyncLog", model).AutoMap(x => x.SysNo).ExecuteReturnLastId<int>("SysNo");
        }
        ...
        public override LiJiaSyncLog GetEntity(int sysNo)
        {
            return Context.Sql("select * from LiJiaSyncLog where SysNo=@SysNo")
                 .Parameter("SysNo", sysNo).QuerySingle<LiJiaSyncLog>();
        }
        ...
        /// <remarks>2013-10-23 黄志勇 创建</remarks>
        public override int Update(LiJiaSyncLog entity)
        {
            return Context.Update("LiJiaSyncLog", entity)
                .AutoMap(o => o.SysNo)
        ...
        {
            return Context.Sql("select count(1) from  LiJiaSyncLog where datamd5=@0", md5).QuerySingle<int>() > 0;
        }
        ...
        public override List<LiJiaSyncLog> GetSyncWaitList(int count)
        {
            #region
            const string command = @"select esl.* from LiJiaSyncLog esl
                          right join (
                                 select * from (
                                   ...
                                   from LiJiaSyncLog e where 
                                    e.flowidentify  not  in (select flowidentify  from LiJiasynclog  where status=@0)  and status=@1
            ...
            var paras = new object[]
            {
               (int)SystemStatus.LiJia同步日志状态.失败, (int)SystemStatus.LiJia同步日志状态.等待同步,
               count,
               (int)SystemStatus.LiJia同步日志状态.失败, (int)SystemStatus.LiJia同步日志状态.等待同步
            };
            return Context.Sql(command, paras).QueryMany<LiJiaSyncLog>();
        }
        ...
        /// <returns></returns>
        /// <remarks>2014-5-4 杨浩 创建</remarks>
        public override List<LiJiaSyncLog> GetSyncFailureList(int count)
        {
            #region command
            const string command = @"select esl.* from LiJiaSyncLog esl
                          right join (
                                 select * from (
                                   select FlowIdentify,max(lastupdatedate) as maxlastupdatedate,(row_number() OVER(ORDER BY max(lastupdatedate) asc)) as row_number 
                                   from LiJiaSyncLog where (status=@0)
                                   group by FlowIdentify        
                                   ) p
            ...
            var paras = new object[]
            {
               (int)SystemStatus.LiJia同步日志状态.失败,
               count,
               (int)SystemStatus.LiJia同步日志状态.失败, (int)SystemStatus.LiJia同步日志状态.等待同步
            };
        ...
        public override List<LiJiaSyncLog> GetRelateList(string flowIdentify)
        {
            string command = @"select * from LiJiaSyncLog  
                                     where flowIdentify=@0 and (status={0} or status={1})
                                     order by FLOWIDENTIFY desc,interfacetype desc,sysno asc ";
            command = string.Format(command, (int)SystemStatus.LiJia同步日志状态.失败, (int)SystemStatus.LiJia同步日志状态.等待同步);

            return Context.Sql(command, flowIdentify).QueryMany<LiJiaSyncLog>();
        }
        /// <summary>
        /// 获取LiJia同步日志
        /// </summary>
        ...
        /// <returns></returns>
        /// <remarks>2014-5-4 杨浩 创建</remarks>
        public override IList<LiJiaSyncLog> GetList(string sysNos)
        {
            string command = @"select * from LiJiaSyncLog  
                                     where  SysNo in ({0}) ";
            command = string.Format(command, sysNos);

            return Context.Sql(command).QueryMany<LiJiaSyncLog>();
        }
        /// <summary>
        /// 获取Kis单据编号
        /// </summary>
        ...
        {
            string command = @"select top 1 VoucherNo from LiJiaSyncLog  
        ...
        {
            string command = "select * from WhStockOut where [Status]=60 and TransactionSysNo not in (select FlowIdentify from LiJiaSyncLog where WarehouseSysNo={0}) and WarehouseSysNo={0} ";
