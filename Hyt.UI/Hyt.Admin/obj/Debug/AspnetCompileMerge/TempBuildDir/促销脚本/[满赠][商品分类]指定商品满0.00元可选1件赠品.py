...
category_sysno = 分类系统编号1(分号)分类系统编号2(注:多个分类系统编号分号分隔)
exclude_product_sysno = 排除商品:商品系统编号1(分号)商品系统编号2(注:多个商品系统编号分号分隔),排除的商品不做计算。
"""
import clr
import System
...
def CheckPromotionProduct(shoppingCart):
    ...
    #当前促销对促销类型为(商品,分类)有效
    if (currSpPromotion.PromotionType != 10):
        return shoppingCart

     #KeyValue
    #参与活动商品
    originalValue_category_sysno = CurrSpPromotion.PromotionRuleKeyValues.FirstOrDefault(lambda kv : kv.RuleKey == "category_sysno").RuleValue
    kv_category_sysno = map(int,originalValue_category_sysno.split(';'))
    ...
    for pdt in shoppingCart:
        ...
        if pdt.IsLock == 1:
            continue
        listCategory = pdt.ProductCategory()
        c = listCategory.Count

        #商品满足当前促销
        if kv_category_sysno.count(pdt.ProductSysNo) != 0:
