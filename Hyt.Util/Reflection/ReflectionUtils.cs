...
namespace Hyt.Util.Reflection
{
    ...
    public class ReflectionUtils
    {
        ...
        public static void Transform(object first, object second)
        {
            if (first != null && second != null)
            {
                PropertyInfo[] ps = first.GetType().GetProperties();
                foreach (var p in ps)
                {
                    var value = p.GetValue(first, null);
                    if (value != null)
                    {
                        var p2 = second.GetType().GetProperty(p.Name);
                        if(p2 != null) 
                            p2.SetValue(second, value, null);
                    }
                } 
            }
        }
