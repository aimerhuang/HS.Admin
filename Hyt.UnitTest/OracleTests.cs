...
namespace Hyt.UnitTest
{
	...
	public class OracleTests 
	{
		...
		{
            get { return new DbContext().ConnectionString("Data Source=myhyt;User Id=scott;Password=pisen@2012;", new OracleProvider()); }
		}
        ...
        public void AddCustomer()
        {
            using (var scope = new TransactionScope())
            {
                try
                {
                    Hyt.DataAccess.CRM.ICrCustomerDao.Instance.CreateCustomer(new Model.CrCustomer { Account="test1", Password="1234" });
                    scope.Complete();
                }
                catch (Exception ex)
                {
