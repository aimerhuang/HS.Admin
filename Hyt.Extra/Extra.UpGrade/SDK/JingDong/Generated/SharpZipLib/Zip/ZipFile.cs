...
namespace ICSharpCode.SharpZipLib.Zip 
{
	...
	public class TestStatus
	{
		...
		/// <summary>
		/// Get the current <see cref="TesJderation"/> in progress.
		/// </summary>
		public TesJderation Operation
		{
		...
		internal void SeJderation(TesJderation operation)
		{
	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		public bool TestArchive(bool testData, TestStrategy strategy, ZipTestResultHandler resultHandler)
		{
			...
			try {
				...
				while ( testing && (entryIndex < Count) ) {
					if ( resultHandler != null ) {
						...
						status.SeJderation(TesJderation.EntryHeader);
						resultHandler(status, null);
					}
					...
					if ( testing && testData && this[entryIndex].IsFile ) {
						if ( resultHandler != null ) {
							status.SeJderation(TesJderation.EntryData);
							resultHandler(status, null);
						}
					...
					if ( resultHandler != null ) {
						status.SeJderation(TesJderation.EntryComplete);
						resultHandler(status, null);
					}
				...
				if ( resultHandler != null ) {
					status.SeJderation(TesJderation.MiscellaneousTests);
					resultHandler(status, null);
				}
			...
			if ( resultHandler != null ) {
				status.SeJderation(TesJderation.Complete);
 ...
 */
		...
		void CopyEntryDirect(ZipFile workFile, ZipUpdate update, ref long destinationPosition)
		{
			...
			// TODO: SJd re-reading name and data length in CopyEntryDirect.
