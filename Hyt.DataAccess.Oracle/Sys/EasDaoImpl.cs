...
namespace Hyt.DataAccess.Oracle.Sys
{
    ...
    /// 2013-10-22 黄志勇 创建
    /// </remarks>
    public class EasDaoImpl : IEasDao
    {
        ...
        public override int GetEasSyncLogCount(int interfaceType, string flowIdentify)
        {
            return Context.Sql("select count(1) from [EasSyncLog]  where  FlowIdentify=@flowIdentify and  InterfaceType=@interfaceType")
        ...
        {
            return Context.Sql("select top 1  VoucherNo from [EasSyncLog]  where  FlowIdentify=@flowIdentify and  InterfaceType=@interfaceType")
               .Parameter("interfaceType", interfaceType)
               .Parameter("flowIdentify", flowIdentify)
               .QuerySingle<string>();
        }
        ...
        public override int EasSyncLogCreate(EasSyncLog model)
        {
            return Context.Insert("EasSyncLog", model).AutoMap(x => x.SysNo).ExecuteReturnLastId<int>("SysNo");
        }
        ...
        /// 修改EAS日志查询方法，仓库编号为多个编号时进行查询（余勇 2014-07-02）
        /// 拼接Sql解决无法使用索引的问题(null is null or 无法使用索引) 2014-09-03 杨浩 修改
        /// </remarks>
        public override Pager<CBEasSyncLog> GetList(ParaEasSyncLogFilter filter)
        {
            #region 短路sql
            ...
            var dataList = Context.Select<CBEasSyncLog>("tb.*").From(sql);
            var dataCount = Context.Select<int>("count(0)").From(sql);

            dataList.Parameters(paras);
            dataCount.Parameters(paras);

            var pager = new Pager<CBEasSyncLog>
            {
        ...
        public override EasSyncLog GetEntity(int sysNo)
        {
            return Context.Sql("select * from EasSyncLog where SysNo=@SysNo")
                 .Parameter("SysNo", sysNo).QuerySingle<EasSyncLog>();
        }
        ...
        /// <remarks>2013-10-23 黄志勇 创建</remarks>
        public override int Update(EasSyncLog entity)
        {
            return Context.Update("EasSyncLog", entity)
                .AutoMap(o => o.SysNo)
        ...
        {
            return Context.Sql("select count(1) from  EasSyncLog where datamd5=@0", md5).QuerySingle<int>() > 0;
        }
        ...
        public override List<EasSyncLog> GetSyncWaitList(int count)
        {
            #region
            const string command = @"select esl.* from EasSyncLog esl
                          right join (
                                 select * from (
                                   ...
                                   from EasSyncLog e where 
                                    e.flowidentify  not  in (select flowidentify  from eassynclog  where status=@0)  and status=@1
            ...
            var paras = new object[]
            {
               (int)SystemStatus.Eas同步日志状态.失败, (int)SystemStatus.Eas同步日志状态.等待同步,
               count,
               (int)SystemStatus.Eas同步日志状态.失败, (int)SystemStatus.Eas同步日志状态.等待同步
            };
            return Context.Sql(command, paras).QueryMany<EasSyncLog>();
        }
        ...
        /// <returns></returns>
        /// <remarks>2014-5-4 杨浩 创建</remarks>
        public override List<EasSyncLog> GetSyncFailureList(int count)
        {
            #region command
            const string command = @"select esl.* from EasSyncLog esl
                          right join (
                                 select * from (
                                   select FlowIdentify,max(lastupdatedate) as maxlastupdatedate,(row_number() OVER(ORDER BY max(lastupdatedate) asc)) as row_number 
                                   from EasSyncLog where (status=@0)
                                   group by FlowIdentify        
                                   ) p
        ...
        public override List<EasSyncLog> GetRelateList(string flowIdentify)
        {
            string command = @"select * from EasSyncLog  
                                     where flowIdentify=@0 and (status={0} or status={1})
                                     order by FLOWIDENTIFY desc,interfacetype desc,sysno asc ";
            command = string.Format(command, (int)SystemStatus.Eas同步日志状态.失败, (int)SystemStatus.Eas同步日志状态.等待同步);

            return Context.Sql(command, flowIdentify).QueryMany<EasSyncLog>();
        }
        /// <summary>
        /// 获取Eas同步日志
        /// </summary>
        ...
        /// <returns></returns>
        /// <remarks>2014-5-4 杨浩 创建</remarks>
        public override IList<EasSyncLog> GetList(string sysNos)
        {
            string command = @"select * from EasSyncLog  
                                     where  SysNo in ({0}) ";
            command = string.Format(command, sysNos);

            return Context.Sql(command).QueryMany<EasSyncLog>();
        }
        /// <summary>
        /// 获取Kis单据编号
        /// </summary>
        ...
        {
            string command = @"select top 1 VoucherNo from EasSyncLog  
        ...
        {
            string command ="select * from WhStockOut where [Status]=60 and TransactionSysNo not in (select FlowIdentify from EasSyncLog where WarehouseSysNo={0}) and WarehouseSysNo={0} ";
        ...
        /// <returns></returns>
        /// <remarks>2017-1-4 杨浩 创建</remarks>
        public override List<EasSyncLog> GetDataByFlowIdentify(int interfaceType, string flowIdentify)
        {
            return Context.Sql("select * from [EasSyncLog]  where  FlowIdentify=@flowIdentify and  InterfaceType=@interfaceType")
               .Parameter("interfaceType", interfaceType)
               .Parameter("flowIdentify", flowIdentify)
               .QueryMany<EasSyncLog>();
        }
    }
}
