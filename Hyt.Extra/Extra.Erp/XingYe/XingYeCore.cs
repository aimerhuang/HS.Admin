...
namespace Extra.Erp.XingYe
{
    ...
    internal sealed class XingYeCore
    {
        #region Private
        ...
        public static XingYeCore Instance
        {
            get { return new XingYeCore(); }
        }
        /// <summary>
        /// 初始化请求数据
        /// </summary>
        ...
        {
            string requestStr = data; //InitializeRequestData(data);
            string apiUrl = "http://211.154.139.8:3312/XiYe/ErpService.svc" + method;  //"http://211.154.139.8:3312/XiYe/ErpService.svc"  config.Gateway
            string exception = "";
            string responseStr = Hyt.Util.MyHttp.PostJsonData(apiUrl, requestStr, ref exception);
            return responseStr;
        }
        ...
        /// <remarks>2018-04-12 罗熙 修改</remarks>
        public static Result<IList<Inventory>> WebInventory(string storageOrgNumber, string[] erpCode, string erpWarehouseSysNo, int warehouseSysNo)
        {
            try
            {
                ...
                var _result = GetApiResponse("/rest/SearchInventory", request.ToJson());
                if (_result==null)
                {
                ...
                JObject jo = (JObject)JsonConvert.DeserializeObject(_result);
                var kisIn = jo["<InventoryList>k__BackingField"].ToString();
                IList<KisInventory> kis = JsonSerializationHelper.JsonToObject<IList<KisInventory>>(kisIn);
                var data = (from item in kis
                            select new Inventory
                            {
                                ...
                                Quantity = Convert.ToInt32(item.FQty),

                            }).ToList();

                return new Result<IList<Inventory>>
                {
                    Status = !Convert.ToBoolean(jo["IsError"].ToString()),
                    StatusCode = jo["ErrCode"].ToString(),
            ...
            catch (Exception e)
            {
                return new Result<IList<Inventory>>
                {
                    ...
                    Status = false,
                    Message =
                        string.Format("{0}。仓库:{1},商品:{2}", e.Message, erpWarehouseSysNo, string.Join(";", erpCode) + ",XingYe:" + e.Message)
                };
            }
        }
        ...
        /// <param name="isAgain">是否重新导入</param>
        /// <param name="enableEas">是否启用XingYe</param>
        /// <param name="isData">是否只取xml数据</param>
        /// <returns></returns>
        /// <remarks>2017-1-4 杨浩 创建</remarks>
        public static Result<string> TransferStocXingYesueBillFacade(Hyt.Model.Generated.XingYeSyncLog easModel, bool isSave = false, bool isAgain = false, bool enableEas = true, bool isData = false)
        {
        ...
        /// 采购入库、退货
        /// </summary>
        /// <param name="easModel">XingYe数据</param> 
        /// <param name="isSave">是否保存为提交状态</param>
        /// <param name="isAgain">是否重新导入</param>
        /// <param name="enableEas">是否启用XingYe</param>
        /// <param name="isData">是否只取xml数据</param>
        ...
        public static Result<string> PurchaseIssueBillFacade(Hyt.Model.Generated.XingYeSyncLog easModel, bool isSave = false, bool isAgain = false, bool enableEas = true, bool isData = false)
        {
            ...
            if (采购状态.退货 == type)
                pOOOrderBillNo = Hyt.DataAccess.Sys.IXingYeDao.Instance.GetVoucherNoByTlowIdentify((int)接口类型.采购入库退货, sale.PurchaseOrderSysNo);
        ...
        public static Result<string> SaleIssueBillFacade(Hyt.Model.Generated.XingYeSyncLog easModel, bool isSave = false, bool isAgain = false, bool enableEas = true, bool isData = false)
        {
            ...
            if (enableEas && isAgain)
            {
                try
                {
                    //实现调用接口
                    //status = 同步状态.等待同步;
                    var _result = GetApiResponse("/rest/AddSellOutBill", json);
                    var response = Hyt.Util.Serialization.JsonUtil.ToObject<Grand.Platform.Api.Contract.Kis.DataContract.AddPoOrderResponse>(_result);
                    status = !response.IsError ? 同步状态.成功 : 同步状态.失败;
                    result.status = !response.IsError;
                    result.message = response.ErrMsg;
            ...
            接口类型 interfaceType, int warehouseSysNo, string remarks, string flowIdentify, decimal voucherAmount, 同步状态 status, bool isAgain)
        {
            ...
            var isImport = Hyt.DataAccess.Sys.IXingYeDao.Instance.IsImport(dataMd5);
            ...
            var log = new Hyt.Model.Generated.XingYeSyncLog()
            {
            ...
            return Hyt.DataAccess.Sys.IXingYeDao.Instance.XingYeSyncLogCreate(log);
        }
