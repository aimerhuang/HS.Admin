...
namespace WxPayAPI
{
    public class MicroPay
    {
        ...
        * @return 刷卡支付结果
        */
        public static Result<WxPayData> Run(string body, string total_fee, string auth_code,string AppId="",string MacId="",string key="")
        {
            ...
            WxPayData result = WxPayApi.Micropay(data, 120, AppId, MacId, key); //提交被扫支付，接收返回结果
		    ...
		    while(queryTimes-- > 0)
		    {
                ...
                WxPayData queryResult = Query(out_trade_no, out succResult, AppId, MacId, key);
			    //如果需要继续查询，则等待2s后继续
			    if(succResult == 2)
                {
				    Thread.Sleep(2000);
