    ...
    internal class PagerBuilder
    {
        ...
        private string GenerateAnchor(PagerItem item)
        {
            ...
            if (_pagerOptions.UseJqueryAjax)
            {
                if (_html.ViewContext.UnobtrusiveJavaScriptEnabled)
                {
                    ...
                    tag.MergeAttribute("pg", item.PageIndex.ToString());
                    tag.MergeAttributes(_ajaxOptions.ToUnobtrusiveHtmlAttributes());
                    var anchor = tag.ToString(TagRenderMode.Normal);
                    
                    //添加自定义包围元素
                    //<remarks>2013-9-5 朱家宏 修改</remarks>
                    if (!string.IsNullOrWhiteSpace(_pagerOptions.AnchorWrapperFormatString))
                    {
                        var surroundTag = new TagBuilder(_pagerOptions.AnchorWrapperFormatString) { InnerHtml = anchor };
                        anchor = surroundTag.ToString(TagRenderMode.Normal);
                    }

                    return string.IsNullOrEmpty(url)
                        ? _html.Encode(item.Text) : anchor;
                }
        ...
        private MvcHtmlString GeneratePagerElement(PagerItem item)
        {
            ...
            if (item.Disabled) //first,last,next or previous page
            {
                ...
                {
                    return CreateWrappedPagerElement(item, String.Format("<a disabled=\"disabled\" class=\"" + _pagerOptions.DisabledCss + " " + _pagerOptions.PrevPageFormatString + "\">{0}</a>", item.Text));
                }
                else if (item.Type == PagerItemType.NextPage)
                {
                    return CreateWrappedPagerElement(item, String.Format("<a disabled=\"disabled\" class=\"" + _pagerOptions.DisabledCss + " " + _pagerOptions.NextPageFormatString + "\">{0}</a>", item.Text));
                }
                else
                {
                    return CreateWrappedPagerElement(item, String.Format("<a disabled=\"disabled\" class=\"" + _pagerOptions.DisabledCss + "\">{0}</a>", item.Text));
                }
            }
            if (item.Type == PagerItemType.PrevPage)
            {
                return CreateWrappedPagerElement(item,
                                                 string.IsNullOrEmpty(url)
                                                     ? _html.Encode(item.Text)
                                                     : String.Format("<a href='{0}' class='{1}'>{2}</a>", url, _pagerOptions.PrevPageFormatString, item.Text));
            }
            else if (item.Type == PagerItemType.NextPage)
            {
                return CreateWrappedPagerElement(item,
                                                 string.IsNullOrEmpty(url)
                                                     ? _html.Encode(item.Text)
                                                     : String.Format("<a href='{0}' class='{1}'>{2}</a>", url, _pagerOptions.NextPageFormatString, item.Text));
            }
            else
            {
                return CreateWrappedPagerElement(item,
        ...
        private MvcHtmlString GenerateJqAjaxPagerElement(PagerItem item)
        {
            ...
            //<remarks>2013-9-5 朱家宏 修改</remarks>
            var disabledAnchor =
                String.Format("<a disabled=\"disabled\" class=\"" + _pagerOptions.DisabledCss + "\">{0}</a>", item.Text);
            if (!string.IsNullOrWhiteSpace(_pagerOptions.AnchorWrapperFormatString))
            {
                var surroundTag = new TagBuilder(_pagerOptions.AnchorWrapperFormatString) { InnerHtml = disabledAnchor };
                disabledAnchor = surroundTag.ToString(TagRenderMode.Normal);
            }

            if (item.Disabled)
                return CreateWrappedPagerElement(item, disabledAnchor);
